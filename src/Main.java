...
public class Main {
    public static void main(String[] args) {
        ...
        List<String> listPath2 = Arrays.asList("save1.dat", "save2.dat", "save3.dat");
    ...
    private static void zipFiles(String path, List<String> list) { // реализация с получением имени архивируемого файла для ZipEntry через метод getFileName созданного объекта Path
        try (ZipOutputStream zout = new ZipOutputStream(new FileOutputStream(path))) {
            for (String arr : list) {
                Path filePath = Paths.get(arr);
    ...
    /*private static void zipFiles(String path, List<String> list) { // реализация с получением имени архивируемого файла для ZipEntry через метод getName созданного объекта File
        try (ZipOutputStream zout = new ZipOutputStream(new FileOutputStream(path))) {
            for (String arr : list) {
